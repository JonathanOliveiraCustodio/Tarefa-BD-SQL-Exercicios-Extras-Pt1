...
--Consultar nome e desconto de 25% no preço dos produtos que custam menos de R$50,00
 SELECT 
  Nome,
  Valor_Unitário,
  'R$ ' + CAST(CAST(Valor_Unitário * 0.75 AS DECIMAL(7,2)) AS VARCHAR(8)) AS "Novo Valor"
  --'R$' + CONVERT(VARCHAR,Valor_Unitário *0.75)  AS "Valor com Desconto 25%"
 FROM Produto
  WHERE Valor_Unitário < 50.00

--Consultar nome e aumento de 10% no preço dos produtos que custam mais de R$100,00				
 SELECT
  Nome,
  Valor_Unitário,
  'R$ ' + CAST(CAST(Valor_Unitário * 1.1 AS DECIMAL(7,2)) AS VARCHAR(8)) AS "Novo Valor"
  FROM Produto 
...
SELECT
    ...
    P.Valor_Unitário AS ValorUnitario,
